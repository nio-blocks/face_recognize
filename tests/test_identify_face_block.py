...
class TestIdentifyFace(NIOBlockTestCase):

    def setUp(self):
        super().setUp()
        sys.modules['cv2'] = MagicMock()
        sys.modules['face_recognition'] = MagicMock()
        from ..identify_face_block import IdentifyFace
        global IdentifyFace

    def test_face_find_known_input(self):
        # The 'known' input does not spit out a signal
        blk = IdentifyFace()
        with patch(IdentifyFace.__module__ + '.base64') as mock_base64, \
                patch(IdentifyFace.__module__ + '.pickle') as mock_pickle:
    ...
    def test_face_find_unknown_input(self):
        ...
        with patch(IdentifyFace.__module__ + '.base64') as mock_base64, \
                patch(IdentifyFace.__module__ + '.face_recognition') as mock_face:
